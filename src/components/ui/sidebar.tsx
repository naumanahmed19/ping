  ...
  return (
    <SidebarContext.Provider value={{ state, open, onOpenChange }}>
      <div
        ...
        className={cn(
          "flex min-h-screen bg-slate-100/50 pl-0 transition-all duration-300 ease-in-out data-[sidebar=closed]:pl-0 sm:pl-[--sidebar-width] dark:bg-slate-800/50",
          className,
        )}
        {...props}
      />
  ...
  ({ className, children }, ref) => {
    ...
    const sidebar = (
      <div
        ref={ref}
        className={cn(
          "flex h-full flex-col border-r bg-white dark:bg-slate-950",
          className,
        )}
      >
        {children}
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn(
        "px-1.5 text-xs font-medium text-slate-500 dark:text-slate-400",
        className,
      )}
      {...props}
    />
